...
export const GameBoard: React.FC<GameBoardProps> = ({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      // Update Combo multiplier timer (decrements frame-rate)
      if (statsRef.current.comboTimer > 0) {
        setStats((prev) => {
          const nextTimer = prev.comboTimer - 1;
          const nextMult = nextTimer <= 0 ? 1 : prev.multiplier;
          return {
            ...prev,
      ...
      switch (type) {
        ...
        case 'slow-ball':
          ...
          ballsRef.current.forEach((b) => {
            b.speed = Math.max(3.2, b.speed - 1.5);
        ...
        case 'fast-ball':
          ...
          color = '#e300ff';
          ballsRef.current.forEach((b) => {
            b.speed = Math.min(8.5, b.speed + 1.2);
            // recalibrate displacement
            const angle = Math.atan2(b.dx, b.dy);
            b.dx = b.speed * Math.sin(angle);
            b.dy = b.speed * Math.cos(angle);
    ...
    const render = (c: CanvasRenderingContext2D) => {
      ...
      bricks.forEach((b) => {
        ...
        c.save();
        // Glow effect for normal/explosive bricks in glowing themes
        if (activeTheme.particleGlow) {
          c.shadowBlur = b.type === 'explosive' ? 12 : 6;
          c.shadowColor = b.type === 'explosive' 
            ? activeTheme.explosive 
            : b.type === 'metal' 
              ? activeTheme.metal 
              : activeTheme.normal[Math.floor(b.x / 100) % activeTheme.normal.length];
        }
        ...
        if (b.type === 'metal') {
          ...
          // Crack lines inside Hard brick depending on missing HP
          if (b.type === 'hard' && b.hp < b.maxHp) {
            c.strokeStyle = 'rgba(255, 255, 255, 0.8)';
            c.beginPath();
            if (b.hp === 2) {
              c.moveTo(b.x + 5, b.y + 5);
              c.lineTo(b.x + b.width / 2, b.y + b.height / 2);
              c.stroke();
            } else if (b.hp === 1) {
              c.moveTo(b.x + 5, b.y + 5);
              c.lineTo(b.x + b.width / 2, b.y + b.height / 2);
              c.lineTo(b.x + b.width - 8, b.y + b.height - 5);
              c.moveTo(b.x + b.width - 12, b.y + 4);
